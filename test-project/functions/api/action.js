...
var isUndefined = (payload) => typeof payload > "u";
...
var isInfinite = (payload) => payload === 1 / 0 || payload === -1 / 0;
...
var simpleRules = [
  ...
  simpleTransformation((v) => v === 0 && 1 / v === -1 / 0, "number", () => {
    return "-0";
  }, Number),
...
var classRule = compositeTransformation(isInstanceOfRegisteredClass, (clazz, superJson) => {
  ...
  if (!clazz)
    throw Error(`Trying to deserialize unknown class '${a[1]}' - check https://github.com/blitz-js/superjson/issues/116#issuecomment-773996564`);
...
var setDeep = (object, path, mapper) => {
  ...
  if (isSet(parent)) {
    ...
    if (oldValue !== newValue)
      parent.delete(oldValue), parent.add(newValue);
  }
  if (isMap(parent)) {
...
function generateReferentialEqualityAnnotations(identitites, dedupe) {
  let result = {}, rootEqualityPaths = undefined;
  if (identitites.forEach((paths) => {
    if (paths.length <= 1)
      return;
    if (!dedupe)
...
var walker = (object, identities, superJson, dedupe, path = [], objectsInThisPath = [], seenObjects = new Map) => {
  ...
  const transformationResult = transformValue(object, superJson), transformed = transformationResult?.value ?? object, transformedValue = isArray(transformed) ? [] : {}, innerAnnotations = {};
  forEach(transformed, (value, index) => {
    if (index === "__proto__" || index === "constructor" || index === "prototype")
      throw Error(`Detected property ${index}. This is a prototype pollution risk, please remove it from your object.`);
    const recursiveResult = walker(value, identities, superJson, dedupe, [...path, index], [...objectsInThisPath, object], seenObjects);
    if (transformedValue[index] = recursiveResult.transformedValue, isArray(recursiveResult.annotations))
...
class SuperJSON {
  ...
  serialize(object) {
    const identities = new Map, output = walker(object, identities, this, this.dedupe), res = { json: output.transformedValue };
  ...
  deserialize(payload, options) {
    let { json, meta } = payload, result = options?.inPlace ? json : copy(json);
    if (meta?.values)
...
async function WrapRequestHandler(context, endpoint) {
  ...
  const parsedData = context.request.method === "GET" || context.request.method === "HEAD" ? paramsFromURL(new URL(context.request.url)) : parseData(context.request.headers.get("content-type") ? await context.request.formData() : undefined), missingProps = endpoint.length - parsedData.length;
  ...
  switch (typeof result) {
    ...
    case "object":
      if (result instanceof Response)
        ...
        return res.headers.set("dataType", "blob"), res.headers.set("Content-Type", result.type), res;
...
var onRequest = async (context) => {
  const method = context.request.method, options = {
